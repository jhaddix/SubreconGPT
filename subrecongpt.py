...
def generate_subdomains(subdomain, domain, api_key):
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Generate 5 subdomain names similar to {subdomain}."},
        ],
    )
...
def main():
    ...
    for line in lines:
        subdomain, domain = line.replace('*', '').split('.', 1)  # Split the line into subdomain and domain
        print(f"\nSubdomain = {subdomain}.{domain}")
        new_subdomains = generate_subdomains(subdomain, domain, args.apikey)
        new_subdomains = [sub.replace(subdomain, '*') for sub in new_subdomains]
